...
function vista_template_vols () {
    ...
    $html = str_replace('{Titulo}', "Vuelos", $html);//Por retocar
    $html = str_replace('{Producto}', General::action_get_product_alone("vols"), $html);//Por retocar
...
function vista_template_product_vols($tipo, $product){
...
function getSecctionVols($html){
    ...
    $html = str_replace('{sectionVols}', 'class="secction"', $html);
...
function vista_buy_vols($tipo, $product, $accion){
    ...
    $html = str_replace('{url}', ucfirst($tipo[0]), $html);//Por retocar
    //$html = str_replace('{formulario}', General::action_form(), $html);
    $html = str_replace('{constructor}', General::action_buy($tipo[0], $product), $html);//Por retocar
    $html = str_replace('{footer}', vista_footer(), $html);
